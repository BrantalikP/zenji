...
const Assignments = BaseCollection.and(z.object({ data: z.array(Assignment) }).passthrough())
...
const Resets = BaseCollection.and(z.object({ data: z.array(Reset) }).passthrough())
...
const Reviews = BaseCollection.and(z.object({ data: z.array(Review) }).passthrough())
const CreateReview = z
  ...
  .passthrough()
const CreateReviewRequest = z.object({ review: CreateReview }).partial().passthrough()
...
const StudyMaterials = BaseCollection.and(z.object({ data: z.array(StudyMaterial) }).passthrough())
const AuxiliaryMeaning = z.object({ meaning: z.string(), type: z.string() }).passthrough()
const Meaning = z
  .object({ meaning: z.string(), primary: z.boolean(), accepted_answer: z.boolean() })
  .passthrough()
...
const BaseReading = z
  .object({ reading: z.string(), primary: z.boolean(), accepted_answer: z.boolean() })
  .passthrough()
...
const ContextSentence = z.object({ en: z.string(), ja: z.string() }).passthrough()
...
const Subject = BaseResource.and(
  z.object({ data: z.union([RadicalSubject, KanjiSubject, VocabSubject]) }).passthrough(),
)
const Subjects = BaseCollection.and(z.object({ data: z.array(Subject) }).passthrough())
const SummaryLessons = z
  .object({ available_at: z.string().datetime({ offset: true }), subject_ids: z.array(z.number()) })
  .passthrough()
...
const User = BaseResource.and(
  z
    .object({
      data: z
        .object({
          current_vacation_started_at: z.string().datetime({ offset: true }).nullable(),
          level: z.number().int(),
...
const VoiceActor = BaseResource.and(
  z
    .object({
      data: z
        .object({ description: z.string(), gender: z.string(), name: z.string() })
        .passthrough(),
    })
    .passthrough(),
)
const VoiceActors = BaseCollection.and(z.object({ data: z.array(VoiceActor) }).passthrough())
...
export const apiHooks = new ZodiosHooks('wanikani', api)
